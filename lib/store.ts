...
import { deriveEventStatusFromTasks } from "./ministry/event-status";
...
const store = globalStore.__emergeMvpStore?.version === 6 ? globalStore.__emergeMvpStore : createInitialStore();
...
function keepSeedDueDateCurrent(value: string, index: number) {
  const dueDate = new Date(value);
  const floor = new Date();
  floor.setDate(floor.getDate() + index + 1);
  floor.setHours(12, 0, 0, 0);
  if (Number.isNaN(dueDate.getTime()) || dueDate.getTime() < floor.getTime()) return floor.toISOString();
  return value;
}

function generateTasksForEvent(event: MinistryEvent, options: { keepSeedTasksCurrent?: boolean } = {}) {
  const templateTasks = defaultTemplateTasks[event.type];
  const generated = templateTasks.map((template, index) => {
    const assignee = users.find((user) => user.role === template.roleAssigned) ?? users[0];
    const plannedDueDate = addDays(event.startTime, template.timelineOffsetDays);
    const task: ActiveTask = {
      id: `task_${event.id}_${template.id}`,
      ministryId: event.ministryId ?? DEFAULT_MINISTRY_ID,
      eventId: event.id,
      taskTitle: template.taskTitle,
      dueDate: options.keepSeedTasksCurrent ? keepSeedDueDateCurrent(plannedDueDate, index) : plannedDueDate,
  ...
  syncEventStatus(event.id);
  return generated;
}

function syncEventStatus(eventId: string) {
  const event = getEvent(eventId);
  if (!event) return;
  const nextStatus = deriveEventStatusFromTasks(tasks.filter((task) => task.eventId === eventId));
  if (nextStatus) event.status = nextStatus;
}

if (!store.initialized) {
  events.forEach((event) => {
    generateTasksForEvent(event, { keepSeedTasksCurrent: true });
    recordActivity({
      type: "event_created",
      eventId: event.id,
...
export function createEvent(input: {
  ...
  const event: MinistryEvent = {
    ...
    autoGeneratedTimeline: [],
    notes: buildEventSupportNotes(input.supportNeeds ?? [], input.supportNotes) || undefined,
  ...
  generateSupportTasksForEvent(event, input.supportNeeds ?? []);
  stagePlanningCenterSpaceOwnerDraft(event, input.plannerRole);
  return getWorkspace(event.id);
}

function generateSupportTasksForEvent(event: MinistryEvent, supportNeeds: EventSupportNeed[]) {
  ...
  supportTaskTitlesForNeeds(supportNeeds).forEach((title) => {
    const task: ActiveTask = {
      ...
      taskTitle: title,
      dueDate,
      assignedUserId: event.contactOwnerId ?? users[0]?.id ?? "",
      status: "todo",
      autoGenerated: true,
    ...
    recordActivity({
      ...
      eventId: event.id,
      taskId: task.id,
      message: `Generated support task: ${task.taskTitle}`,
...
function stagePlanningCenterSpaceOwnerDraft(event: MinistryEvent, plannerRole?: string) {
  const draft = buildPlanningCenterSpaceOwnerDraft(event, users, plannerRole);
  if (!draft) return;
  communications.unshift(draft);
  recordActivity({
    type: "communication_preview_generated",
    eventId: event.id,
    message: `Drafted Planning Center space-owner email for ${event.title}`,
    metadata: { sent: false, integrationType: "planning_center" }
  });
}
