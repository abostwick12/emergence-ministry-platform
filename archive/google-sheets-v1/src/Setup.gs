...
function initializeSystemSettings() {
  ...
  sheet.getRange('A1:F1').merge().setValue(EMERGENCE_APP.name)
    .setBackground(UI_COLORS.SYSTEM_SETTINGS)
    .setFontColor('#FFFFFF')
    .setFontWeight('bold')
    .setFontSize(18);
  sheet.getRange('A2:F2').merge().setValue('System Settings - configuration placeholders only. Do not store secrets here.')
    .setBackground(UI_COLORS.SOFT_BG)
    .setFontColor(UI_COLORS.MUTED);
  sheet.getRange(4, 1, 1, 4).setValues([['Section', 'Setting', 'Value', 'Notes']])
    .setBackground(UI_COLORS.SYSTEM_SETTINGS)
    .setFontColor('#FFFFFF')
    .setFontWeight('bold');

  const existing = getSettingsMap();
  const rows = REQUIRED_SETTINGS.map((setting) => [
    setting.section,
    setting.key,
    Object.prototype.hasOwnProperty.call(existing, setting.key) && existing[setting.key] !== '' ? existing[setting.key] : setting.value,
    setting.notes
  ]);
...
function seedDropdownValues() {
  const sheet = getOrCreateSheet_(SHEET_NAMES.SYSTEM_SETTINGS);
  const startRow = 24;
  sheet.getRange(startRow, 1, 1, 3).setValues([['Dropdown Lists', 'Named Range', 'Values']])
...
function initializeNamedRanges() {
  ...
  DROPDOWN_SOURCES.forEach((source, index) => {
    existingRanges
      .filter((namedRange) => namedRange.getName() === source.rangeName)
      .forEach((namedRange) => namedRange.remove());
    const row = 25 + index;
    spreadsheet.setNamedRange(source.rangeName, sheet.getRange(row, 3, 1, source.values.length));
  });
}
...
function runPackageOneSmokeTest() {
  ...
  checks.push(['Required settings keys exist', REQUIRED_SETTINGS.every((setting) => Object.prototype.hasOwnProperty.call(getSettingsMap(), setting.key))]);
  ...
  SpreadsheetApp.getUi().alert(
    failed.length
      ? `Package 1 smoke test found ${failed.length} issue(s): ${failed.map((check) => check[0]).join(', ')}`
      : 'Package 1 smoke test passed basic structure checks.'
  );
...
function setupSimpleShellTable_(sheetName, title, subtitle, headers, accentColor) {
  ...
  sheet.clearFormats();
  sheet.setTabColor(TAB_COLORS[sheetName]);
  sheet.getRange(1, 1, 1, Math.max(headers.length, 6)).merge().setValue(title)
    .setBackground(accentColor)
    .setFontColor('#FFFFFF')
    .setFontWeight('bold')
    .setFontSize(18);
  sheet.getRange(2, 1, 1, Math.max(headers.length, 6)).merge().setValue(subtitle)
    .setBackground(UI_COLORS.SOFT_BG)
    .setFontColor(UI_COLORS.MUTED);
...
function setupWorshipSheet_() {
  ...
  sheet.getRange('A1:Z1').merge().setValue('WORSHIP')
    .setBackground(UI_COLORS.WORSHIP)
    .setFontColor('#FFFFFF')
    .setFontWeight('bold')
    .setFontSize(18);
  sheet.getRange('A2:Z2').merge().setValue('See the Core 10 library and two-month worship plan at a glance.')
    .setBackground(UI_COLORS.SOFT_BG)
    .setFontColor(UI_COLORS.MUTED);
  sheet.getRange(6, 1, 1, WORSHIP_LIBRARY_COLUMNS.length).merge().setValue('Core 10 Song Library')
    .setBackground('#FCE7EF')
    .setFontColor(UI_COLORS.WORSHIP)
    .setFontWeight('bold');
  sheet.getRange(TABLE_START_ROWS.WORSHIP_LIBRARY, 1, 1, WORSHIP_LIBRARY_COLUMNS.length).setValues([WORSHIP_LIBRARY_COLUMNS])
    .setBackground(UI_COLORS.WORSHIP)
    .setFontColor('#FFFFFF')
    .setFontWeight('bold');
  sheet.getRange(17, 1, 1, WORSHIP_PLAN_COLUMNS.length).merge().setValue('Two-Month Worship Plan')
    .setBackground('#FCE7EF')
    .setFontColor(UI_COLORS.WORSHIP)
    .setFontWeight('bold');
  sheet.getRange(TABLE_START_ROWS.WORSHIP_PLAN, 1, 1, WORSHIP_PLAN_COLUMNS.length).setValues([WORSHIP_PLAN_COLUMNS])
    .setBackground(UI_COLORS.WORSHIP)
    .setFontColor('#FFFFFF')
    .setFontWeight('bold');
...
function applyGenericValidations_() {
  applyColumnValidation_(SHEET_NAMES.DOCUMENTS, 'Document Type', NAMED_RANGES.DOCUMENT_STATUSES);
...
function formatWorkbookDefaults_() {
  TAB_ORDER.forEach((sheetName) => {
    ...
    sheet.setHiddenGridlines(sheetName === SHEET_NAMES.EVENTS || sheetName === SHEET_NAMES.KANBAN);
  });
}
